...
public class UnidadAsignadaDAO {
    ...
    public UnidadAsignadaDTO obtenerUnidadPorNombre(String nombre) {
        try {
            Class.forName(DRIVER);
            Connection conexion = DriverManager.getConnection(URL, USUARIO, CLAVE);
            Statement declaracion = conexion.createStatement();
            ResultSet resultado = declaracion.executeQuery("SELECT ASSIGNED_UNIT.ID,ASSIGNED_UNIT.ID_INTERNALUNIT, ASSIGNED_UNIT.ID_COMPANY,ASSIGNED_UNIT.ID_SUPERIOR_UNIT FROM ASSIGNED_UNIT JOIN INTERNAL_UNIT ON ASSIGNED_UNIT.ID_INTERNALUNIT=INTERNAL_UNIT.ID WHERE INTERNAL_UNIT.NAME = '" + nombre + "'");
            while (resultado.next()) {
                this.setId(resultado.getInt(1));
                this.setId_internalUnit(resultado.getInt(2));
                this.setId_company(resultado.getInt(3));
        ...
        return new UnidadAsignadaDTO(this.getId(), this.getId_internalUnit(), this.getId_company(), this.getId_superior_unit());
    }
